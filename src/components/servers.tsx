...
function AddServer(props) {
  ...
  return <div id={props.name} className='addServer flex flex-row items-center gap-2 relative'>
    <div className={`${props.isClicked ? 'h-[40px]' : 'h-[8px]'} ${isHoverd ? 'h-[40px]' : 'h-[8px]'} h-[40px] w-[4px] bg-white rounded-r-[4px] fixed left-0 ease-in-out duration-300`}></div>
...
function DiscoverServers(props) {
  const [isHoverd, setIsHoverd] = useState(false)
